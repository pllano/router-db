...
Если база указанная в конфигурации `$resource` недоступна, `routerDb` переключит на `master` или `slave` базу, а `routerDb\Queue` запишет запрос в очередь запросов. Как только основная база ресурса станет доступна `routerDb\Queue` синхронизирует данные.
