...
Если при запросах `POST` `PUT` `PATCH` `DELETE` использовалась не основная база данных ресурса `routerDb\Db` запишет запрос в резервную базу `slave` и таблицу запросов `queue.json`
