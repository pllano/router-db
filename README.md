...
Если при запросе использовалась не основная база данных ресурса `routerDb\Db` запишет запрос в резервную базу `slave` и таблицу запросов `queue.json`
...
- Максимальное разделение фронтенда и бэкенда. Например, при использовании фронтенд-фреймворков. Например подключить фронтенд-приложение на Angular.js
