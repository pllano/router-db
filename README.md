...
Автоматическое создание таблиц (ресурсов) из файла [db.json](https://github.com/pllano/structure-db/blob/master/db.json).

Вы можете настроить свою структуру для каждой таблицы (ресурса) отредактировав файл db.json и скопировав его в директорию базы данных jsonDb `/../_json_db_/_db_/core/`
...
- `field name *` - любое поле которое есть в ресурсе. Параметров `field name *` может быть несколько
...
- `operator` - Разделитель `AND` или `OR`. По умолчанию `OR`
- `limit` - лимит записей на страницу. По умолчанию `30`
- `offset` - смещение (с какой записи выводим). По умолчанию `0`
- `order` - сотрировка `DESC` или `ASC`. По умолчанию `DESC`
- `sort` - поле по которому сортируем. По умолчанию `id`
- `type` - Тип поиска. По умолчанию `LIKE`. (Все возможности еще в разработке !). Если база данных или библиотека не поддерживает указанный тип поиска он должен игнорироваться.  Например: `simple_query_string` поддерживается в Elasticsearch, но не поддерживается в других базах данных, таким образом при запросе к другим базам должен либо быть проигнорирован либо сведен к максимально похожему типу поиска, в этом случае к `LIKE`.
