...
С `routerDb` вы можете писать один код для работы со всеми базами данных `mysql`  `elasticsearch` `json` или работу через `api`
...
Имеет встроенный роутер переключения между базами данных на лету, если база данных необходимого ресурса указанная в конфигурации `$resource` недоступна, `routerDb` переключит на `master` или `slave` базу.
### Писать запросы в резервную базу `slave` когда основная недоступна
`routerDb\Queue` запишет запрос в очередь запросов если при запросе использовалась не основная база данных ресурса. Как только основная база ресурса станет доступна `routerDb\Queue` синхронизирует данные.
### Синхронизировать данные между `master` и `slave`
Когда основная база `master` снова доступна синхронизировать данные в базу указанную в конфигурации `$resource` или `master` если нет.
